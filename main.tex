...
\renewcommand*{\arraystretch}{1.5}
    \centering
    \caption{Summary of Software Packages}
    \label{tab:my_label}
    \footnotesize
    ...
    PROPER & Diffraction simulation & \cite{krist_proper:_2007,krist_wfirst_2017} & \url{https://github.com/ajeldorado/proper-models/tree/master/wfirst_cgi}\\
     ...
     direct-imaging-sims + model spectra & Target simulation &\citenum{Lacy2019, lacy_prospects_2020}  & \url{https://github.com/blacy/direct-imaging-sims},
...
Based on FALCO\cite{riggs2018falco1}, the ``Lightweight Space Coronagraph Simulator''\footnote{\url{https://github.com/leonidprinceton/LightweightSpaceCoronagraphSimulator}} computes small linear perturbations about the nominal dark hole instead of propagating the full optical model.
...
\begin{figure}[ht]
    ...
    \caption{Images from publicly available OS9  simulations of the \gls{HLC}  including time dependent speckle and detector effects. Left: simulated image with injected planets. Right: reference subtracted image showing planets and residual speckles.}
...
%update:
The effect of the coronagraph transfer functions on exozodiacal dust models is shown through the simulations using publicly available PSF cubes from IPAC. These simulations are done by generating the dust models of various parameters and applying the \gls{HLC}’s transfer function via matrix multiplication. Because of the evolution of the PSF for angular offsets, PSFs must be generated for every angular offset of the pixel coordinates of the dust model. These PSFs are generated via interpolation of the PSFs available from IPAC and stored as matrices in memory. The exozodiacal model can then be flattened into a vector rather than a 2D array and multiplied by the matrix of interpolated PSFs since the HLC is still assumed to be a linear system. This allows for fast simulations of a variety of models. For details and examples of such disk generation, see Milani et al. \cite{milani_faster_2020}. 
...
In addition to the simulation software describe above, Data Challenges\cite{hildebrandt_wfirst_2018} have allowed the community to engage and develop additional tools\footnote{\url{https://roman.ipac.caltech.edu/sims/Exoplanet_Data_Challenges.html}}.
Integral field spectrograph modes were considered for Roman and simulated in Coronagraph and Rapid Imaging Spectrograph in Python (\verb+crispy+)\cite{rizzo_simulating_2017} which may also prove useful for other missions\footnote{\url{https://github.com/mjrfringes/crispy}}. 
It is hoped that the availability of these tools will allow the community to develop science questions with Roman CGI and provide springboards for future coronagraphic missions.
